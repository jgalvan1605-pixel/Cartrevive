...
export class HoldedService {
    async createOpportunity(cart, config) {
        if (!config.apiKey) {
        ...
        const tags = ['CartRevive', 'Carrito Abandonado'];
        if (amount >= 500)
            tags.push('VIP Ticket');
        if (contactPhone)
            tags.push('Contacto Telefónico');
        const payload = {
    ...
    async markOpportunityWon(dealId, apiKey, wonStageId) {
